...
export async function ProjectLinkOverlay({
  ...
  if (overlayType === "stores") {
    return (
      <div className="flex flex-col items-center justify-center gap-2">
        <StoreLink href={appStoreUrl} icon={<AppStoreLogo className="w-3.5 h-3.5 flex-shrink-0" />} label={t("appStore")} />
        <StoreLink href={playStoreUrl} icon={<GooglePlayLogo className="w-3.5 h-3.5 flex-shrink-0" />} label={t("googlePlay")} />
      </div>
    )
  }

  if (overlayType === "live-stores") {
    return (
      <div className="flex flex-col items-center justify-center gap-2">
        <StoreLink href={url} icon={<ExternalLinkIcon className="w-3.5 h-3.5 flex-shrink-0" />} label={t("viewLive")} />
        <StoreLink href={appStoreUrl} icon={<AppStoreLogo className="w-3.5 h-3.5 flex-shrink-0" />} label={t("appStore")} />
        <StoreLink href={playStoreUrl} icon={<GooglePlayLogo className="w-3.5 h-3.5 flex-shrink-0" />} label={t("googlePlay")} />
      </div>
    )
  ...
  // live-lg
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="glass px-5 py-2.5 rounded-xl text-sm font-semibold text-white hover:text-amber-400 transition-colors cursor-pointer flex items-center gap-2">
